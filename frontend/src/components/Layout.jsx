 ...
 * Provides the shared navigation bar and account status notifications.
 */
      ...
      <nav className={`navbar navbar-expand-lg border-bottom sticky-top ${theme === 'dark' ? 'navbar-dark bg-black' : 'navbar-light bg-white'}`}>
        <div className="container-fluid px-4">
          ...
          <Link to="/" className="navbar-brand d-flex align-items-center gap-2 text-decoration-none">
            ...
            <span className={`fw-bold mb-0 h4 ${theme === 'dark' ? 'text-white' : 'text-dark'}`}>
              NoteMate
            </span>
          </Link>
              ...
              <>
                <div className="d-flex align-items-center gap-2">
                  {user.avatarUrl ? (
                    ...
                    <div className={`rounded-circle border d-flex align-items-center justify-content-center ${theme === 'dark' ? 'bg-secondary' : 'bg-light'}`} style={{ width: "32px", height: "32px" }}>
                      <small>👤</small>
                    </div>
                  )}
